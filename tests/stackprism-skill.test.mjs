...
test('stackprism site experience skill advertises the current bridge workflow', () => {
  ...
  assert.match(skillSource, /Accept a user-provided redacted screenshot or recording only if the user has already removed private content/)
  ...
  assert.match(skillSource, /STACKPRISM_CAPTURE_TARGET_FLAGS='--allow-private-network'/)
  assert.match(skillSource, /\$\{STACKPRISM_CAPTURE_TARGET_FLAGS:-\}/)
...
test('stackprism site experience skill points agents to repo-local references', () => {
  ...
  assert.match(consumptionGuideSource, /user-supplied redacted screenshot or recording/)
  ...
  assert.match(consumptionGuideSource, /Accept redacted screenshots or recordings only when the user has already removed private content/)
...
test('repo-local skill documents bridge asset parity and local installation boundary', () => {
  ...
  assert.match(readmeSource, /Do not capture login-protected, account-specific/)
  assert.match(readmeSource, /already-redacted screenshot or recording/)
  assert.match(readmeSource, /current browser page without a URL/)
  assert.match(readmeSource, /Retry commands must preserve the original capture context/)
  assert.match(readmeSource, /browser\/profile opener env such as `STACKPRISM_BROWSER_OPEN_COMMAND`/)
  assert.match(readmeSource, /STACKPRISM_CAPTURE_TARGET_FLAGS='--allow-private-network'/)
  assert.match(readmeSource, /--max-resource-urls 150/)
  assert.match(readmeSource, /--max-resource-urls 50 --no-screenshot/)
  assert.match(readmeSource, /cannot support visual parity/)
