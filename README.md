...
[![Platform](https://img.shields.io/badge/Platform-Chrome%20%7C%20Edge%20%7C%20Firefox%20128%2B-26A69A)](#安装)
[![Feedback](https://img.shields.io/badge/Feedback-Upstream%20Issues-FF6F61)](https://github.com/setube/stackprism/issues)
...
普通用户可优先使用上游 [Chrome Web Store](https://chromewebstore.google.com/detail/stackprism/cagpdifljieeiajlhlcboelglkalofak)、[Edge Add-ons](https://microsoftedge.microsoft.com/addons/detail/stackprism/ojgmhlogaoiegdonnlnibeoikbleccno) 或 [GitHub Release](https://github.com/setube/stackprism/releases)。完整步骤见 [安装与启用](docs/guide/install.md)。

需要当前 fork 的 Agent Bridge、repo-local skill 或 Firefox 本地验证时，从源码加载：
...
当前 fork 文档不提供 Firefox Add-ons 商店入口。Firefox 128+ 开发验证优先临时加载 `dist-firefox/manifest.json`。`release/` 里的 `.xpi` 是打包产物；普通 Firefox 正式版永久安装仍以 Firefox Add-ons 签名或对应发布渠道为准。
...
Agent Bridge 是当前 fork 源码构建版包含的默认关闭本机能力。用户在扩展设置中显式启用后，本机 AI Agent 可以通过 `127.0.0.1` bridge 获取 `stackprism.site_experience_profile.v1`，用于参考目标站点的技术、视觉、布局、组件、交互、UX 和资源摘要。
...
本机开发目标、`localhost`、`127.0.0.1`、私网地址和真实内网目标需要同时在扩展设置中开启高风险网络目标选项，并在 helper 请求中使用 `--allow-private-network`。直接调用 bridge API 或 Python fallback 时，对应字段是 capture request 里的 `options.allowPrivateNetworkTarget = true`。不要复用被 `PRIVATE_NETWORK_TARGET_BLOCKED` 拒绝的旧 bridge URL。
...
- **首次安装请刷新目标页**:让 background 捕获主文档响应头
...
- **Bug / 误识 / 漏识反馈**:[上游 Issues](https://github.com/setube/stackprism/issues) — popup 上直接点「识别不准确」按钮会自动填好上游议题模板
- **规则贡献**:扩展设置页点「提交规则贡献」,或直接 PR 到 [`public/rules/`](public/rules/)
- **Agent Bridge / 当前 fork 问题**:当前 fork 未开启公开 Issues / Discussions，请在相关 PR 线程或维护者指定渠道反馈，并说明 fork commit、浏览器 profile、是否启用 Agent Bridge，以及是否使用 repo-local skill
- **讨论与提案**:[上游 Discussions](https://github.com/setube/stackprism/discussions)
