...
- Agent Bridge — 启用后允许本机 Agent Bridge 读取当前浏览器可观测的技术与体验摘要
...
Agent Bridge 启用状态是例外：它只存在当前浏览器 profile 的 `chrome.storage.local`，不会随 Chrome sync 同步到其他设备、浏览器或 profile。换环境后需要重新显式开启。
